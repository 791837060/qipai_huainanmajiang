	...
	public JuResultVO(JuResultDbo juResultDbo, PukeGameDbo pukeGameDbo) {
		...
		if (juResult.getPlayerResultList() != null) {
			juResult.getPlayerResultList().forEach((juPlayerResult) -> playerResultList.add(
					new PaodekuaiPlayerResultVO(juPlayerResult, pukeGameDbo.findPlayer(juPlayerResult.getPlayerId()),pukeGameDbo.getOptionalPlay())));
		} else {
			pukeGameDbo.getPlayers().forEach((pukeGamePlayerDbo) -> playerResultList.add(new PaodekuaiPlayerResultVO(pukeGamePlayerDbo)));
		}
	}
