...
public class PanResultVO {
	...
	public PanResultVO(PanResultDbo panResultDbo, PukeGameDbo pukeGameDbo) {
		...
		List<PaodekuaiPlayerValueObject> players = panResultDbo.getPanActionFrame().getPanAfterAction().getPaodekuaiPlayerList();
		if (list != null) {
			...
			list.forEach((panPlayerResult) -> {
				...
				playerResultList.add(new PaodekuaiPanPlayerResultVO(pukeGameDbo.findPlayer(panPlayerResult.getPlayerId()), panPlayerResult, paodekuaiPlayer));
			});
		}
