...
public class PukeHistoricalJuResult {
	...
	public PukeHistoricalJuResult(JuResultDbo juResultDbo, PukeGameDbo pukeGameDbo) {
		...
		if (juResult.getPlayerResultList() != null) {
			juResult.getPlayerResultList().forEach((juPlayerResult) -> playerResultList.add(
					new PaodekuaiJuPlayerResultMO(juPlayerResult, pukeGameDbo.findPlayer(juPlayerResult.getPlayerId()),pukeGameDbo.getOptionalPlay())));
		} else {
			pukeGameDbo.getPlayers().forEach((pukeGamePlayerDbo) -> playerResultList.add(new PaodekuaiJuPlayerResultMO(pukeGamePlayerDbo)));
		}

//		if (pukeGameDbo.getOptionalPlay().isJinyuanzi()) {   //进园子玩法下最后得分要减去游戏开始时给的园子分
//			for (PaodekuaiJuPlayerResultMO paodekuaiJuPlayerResultMO : playerResultList) {
//				double totalScore = paodekuaiJuPlayerResultMO.getTotalScore();
//				totalScore -= pukeGameDbo.getOptionalPlay().getYuanzifen();
//				paodekuaiJuPlayerResultMO.setTotalScore(totalScore);
//			}
//		}
