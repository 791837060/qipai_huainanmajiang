...
import com.anbang.qipai.qinyouquan.cqrs.q.dao.AllianceDboDao;
...
public class MongodbAllianceDboDao implements AllianceDboDao {
    ...
    @Override
    public long countByAgentIdAndLianmengId(String mengzhu, String lianmengId) {
        Query query = new Query();
    ...
    @Override
    public List<AllianceDbo> findByAgentIdAndLianmengId(String mengzhu, String lianmengId) {
        Query query = new Query();
        if (!StringUtils.isEmpty(mengzhu)) {
            query.addCriteria(Criteria.where("mengzhu").is(mengzhu));
        }
    ...
    @Override
    public long countByConditions(String mengzhu, String nickname, String phone, String userName) {
        Query query = new Query();
        if (!StringUtils.isEmpty(mengzhu)) {
            query.addCriteria(Criteria.where("mengzhu").is(mengzhu));
        }
    ...
    @Override
    public List<AllianceDbo> findByConditions(int page, int size, String mengzhu, String nickname, String phone, String userName) {
        Query query = new Query();
        if (!StringUtils.isEmpty(mengzhu)) {
            query.addCriteria(Criteria.where("mengzhu").is(mengzhu));
        }
    ...
    @Override
    public long countAmountByAgentId(String mengzhu) {
        Query query = new Query();
        query.addCriteria(Criteria.where("mengzhu").is(mengzhu));
        return mongoTemplate.count(query, AllianceDbo.class);
    }
    ...
    @Override
    public List<AllianceDbo> findByAgentId(int page, int size, String mengzhu) {
        Query query = new Query();
        query.addCriteria(Criteria.where("mengzhu").is(mengzhu));
        query.skip((page - 1) * size);
        query.limit(size);
