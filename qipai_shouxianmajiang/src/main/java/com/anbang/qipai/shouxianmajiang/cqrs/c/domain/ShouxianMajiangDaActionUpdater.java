 ...
 * @author lsc
 */
public class ShouxianMajiangDaActionUpdater implements MajiangPlayerDaActionUpdater {
        ...
        while (true) {
            if (!xiajiaPlayer.getId().equals(daAction.getActionPlayerId())) { //出牌玩家不是下家玩家
                ...
                if (!canNotHuPlayers.contains(xiajiaPlayer.getId()) && dianpao) { //点炮胡
                    ShouxianMajiangPanResultBuilder tuidaoHuPanResultBuilder = (ShouxianMajiangPanResultBuilder) ju.getCurrentPanResultBuilder();
                    ...
                    ShouxianMajiangHu bestHu = ShouxianMajiangJiesuanCalculator.calculateBestDianpaoHu(couldDihu && !currentPan.getZhuangPlayerId().equals(xiajiaPlayer.getId()),
...
//                        //非胡牌型特殊胡---十三幺(听胡)
//                        boolean shisanyaoPaixing = ShouxianMajiangJiesuanCalculator.isShisanyaoPaixing(xiajiaPlayer,daAction.getPai());
//                        if (shisanyaoPaixing) {
