...
public class ZongyangMajiangGuoActionUpdater implements MajiangPlayerGuoActionUpdater {
    ...
    public void updateActions(MajiangGuoAction guoAction, Ju ju) {
        ...
        if (action.getType().equals(MajiangPlayerActionType.mo)) { //过的是我摸牌之后的胡,杠
            ...
            if (currentPan.allPlayerHasNoActionCandidates() && !currentPan.anyPlayerHu()) {// 如果所有玩家啥也干不了
                ZongyangMajiangPengGangActionStatisticsListener pengGangRecordListener = ju.getActionStatisticsListenerManager().findListener(ZongyangMajiangPengGangActionStatisticsListener.class);
