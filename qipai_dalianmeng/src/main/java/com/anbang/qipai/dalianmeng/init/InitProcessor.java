package com.anbang.qipai.dalianmeng.init;


import com.anbang.qipai.dalianmeng.cqrs.c.service.disruptor.CoreSnapshot;
import com.anbang.qipai.dalianmeng.cqrs.c.service.disruptor.FileUtil;
import com.anbang.qipai.dalianmeng.cqrs.c.service.disruptor.ProcessCoreCommandEventHandler;
import com.anbang.qipai.dalianmeng.cqrs.c.service.disruptor.SnapshotJsonUtil;
...
public class InitProcessor {
	...
	private void recover() throws Throwable {
		...
		try {
			memberSnapshot = (CoreSnapshot) snapshotJsonUtil.recovery(coreCommandEventHandler.getSnapshotFileBasePath(), CoreSnapshot.class);
		} catch (IOException e) {
			e.printStackTrace();
		}
		if (memberSnapshot != null) {
			singletonEntityRepository.fill((SingletonEntityRepository) memberSnapshot.getContentMap().get(SingletonEntityRepository.class));
		}
	...
	private void invokeCommands(List<Command> commands) throws ClassNotFoundException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		for (Command command : commands) {
			if (command instanceof CommonCommand) {
				...
				if (cmd.getParameters() != null && cmd.getParameters().length > 0) {
					try {
						service.getClass().getMethod(cmd.getMethod(), cmd.getParameterTypes()).invoke(service, cmd.getParameters());
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else {
