...
import com.anbang.qipai.dalianmeng.msg.sink.YizhengMajiangGameSink;
...
@EnableBinding(YizhengMajiangGameSink.class)
public class YizhengGameMsgReceiver {
    @Autowired
    ...
    @StreamListener(YizhengMajiangGameSink.YIZHENGMAJIANGGAME)
    public void receive(CommonMO mo) {
        ...
        if ("playerQuit".equals(msg)) {// 有人退出游戏
            ...
            if (table != null) {
                playService.gamePlayerQuitQame(Game.yizhengMajiang, gameId, playerId);
            }
        ...
        if ("ju canceled".equals(msg)) {// 取消游戏
            Map data = (Map) mo.getData();
            String gameId = (String) data.get("gameId");
            GameTable gameTable = playService.findTableByGameAndServerGameGameId(Game.yizhengMajiang, gameId);
            ...
            if (gameTable != null) {
                ...
                playService.gameTableFinished(Game.yizhengMajiang, gameId);
            }
        }
        ...
        if ("pan finished".equals(msg)) {// 一盘游戏结束
            ...
            GameTable gameTable = playService.findTableByGameAndServerGameGameId(Game.yizhengMajiang, gameId);
            if (gameTable != null) {
                playService.panFinished(Game.yizhengMajiang, gameId, no, playerIds);
            }
        }
        if ("game start".equals(msg)) {// 游戏开始
            ...
            GameTable gameTable = playService.findTableByGameAndServerGameGameId(Game.yizhengMajiang, gameId);
            if (gameTable != null) {
                playService.gameTablePlaying(Game.yizhengMajiang, gameId);
            }
        }
        if ("game delay".equals(msg)) {// 游戏延时
            ...
            if (gameTable != null) {
                // 延时1小时
                playService.delayGameTable(Game.yizhengMajiang, gameId, gameTable.getDeadlineTime() + 1 * 60 * 60 * 1000);
            }
        }
    }
}
