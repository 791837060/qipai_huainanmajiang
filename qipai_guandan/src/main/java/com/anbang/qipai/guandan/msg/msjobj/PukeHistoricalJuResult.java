...
import com.anbang.qipai.guandan.web.vo.PanResultVO;
...
public class PukeHistoricalJuResult {
	...
	private List<GuandanJuPlayerResultMO> playerResultList;
	...
	public PukeHistoricalJuResult(JuResultDbo juResultDbo, PukeGameDbo pukeGameDbo) {
		...
		GuandanJuResult juResult = juResultDbo.getJuResult();
		...
		if (juResult.getPlayerResultList() != null) {
			juResult.getPlayerResultList().forEach((juPlayerResult) -> playerResultList.add(new GuandanJuPlayerResultMO(juPlayerResult, pukeGameDbo.findPlayer(juPlayerResult.getPlayerId()))));
		} else {
			pukeGameDbo.getPlayers().forEach((pukeGamePlayerDbo) -> playerResultList.add(new GuandanJuPlayerResultMO(pukeGamePlayerDbo)));
		}

//		if (pukeGameDbo.getOptionalPlay().isJinyuanzi()) {   //进园子玩法下最后得分要减去游戏开始时给的园子分
//			for (GuandanJuPlayerResultMO tianchangxiaohuaJuPlayerResultMO : playerResultList) {
//				double totalScore = tianchangxiaohuaJuPlayerResultMO.getTotalScore();
//				totalScore -= pukeGameDbo.getOptionalPlay().getYuanzifen();
//				tianchangxiaohuaJuPlayerResultMO.setTotalScore(totalScore);
//			}
//		}
	...
	public List<GuandanJuPlayerResultMO> getPlayerResultList() {
		return playerResultList;
	}

	public void setPlayerResultList(List<GuandanJuPlayerResultMO> playerResultList) {
