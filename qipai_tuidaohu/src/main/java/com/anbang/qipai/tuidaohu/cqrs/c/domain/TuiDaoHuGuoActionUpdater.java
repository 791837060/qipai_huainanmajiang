...
public class TuiDaoHuGuoActionUpdater implements MajiangPlayerGuoActionUpdater {
    ...
    public void updateActions(MajiangGuoAction guoAction, Ju ju) {
        ...
        if (action.getType().equals(MajiangPlayerActionType.mo)) { //过的是我摸牌之后的胡,杠
            ...
            if (currentPan.allPlayerHasNoActionCandidates() && !currentPan.anyPlayerHu()) {// 如果所有玩家啥也干不了
                TuiDaoHuPengGangActionStatisticsListener pengGangRecordListener = ju.getActionStatisticsListenerManager().findListener(TuiDaoHuPengGangActionStatisticsListener.class);
