 ...
 * @author lsc
 */
public class TuiDaoHuDaActionUpdater implements MajiangPlayerDaActionUpdater {
        ...
        while (true) {
            if (!xiajiaPlayer.getId().equals(daAction.getActionPlayerId())) { //出牌玩家不是下家玩家
                ...
                if (!canNotHuPlayers.contains(xiajiaPlayer.getId()) && dianpao) { //点炮胡
                    ...
                    TuiDaoHuHu bestHu = TuiDaoHuJiesuanCalculator.calculateBestDianpaoHu(couldDihu && !currentPan.getZhuangPlayerId().equals(xiajiaPlayer.getId()),
