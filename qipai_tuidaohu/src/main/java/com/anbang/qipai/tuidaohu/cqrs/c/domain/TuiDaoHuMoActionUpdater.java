...
public class TuiDaoHuMoActionUpdater implements MajiangPlayerMoActionUpdater {
    @Override
    public void updateActions(MajiangMoAction moAction, Ju ju) throws Exception {
        ...
        if (guipaiCount == 4) {
            ...
            hufen.calculate();
            TuiDaoHuHu sancaishenHu = new TuiDaoHuHu(hufen);
            sancaishenHu.setZimo(true);
            player.addActionCandidate(new MajiangHuAction(player.getId(), sancaishenHu));
        }else {
            TuiDaoHuHu bestHu = TuiDaoHuJiesuanCalculator.calculateBestZimoHu(couldTianhu, gouXingPanHu, player, moAction,optionalPlay);//计算时候胡牌
            if (bestHu != null) {
                bestHu.setZimo(true);
