...
public class MongodbLianmengApplyDao implements LianmengApplyDao {
	...
	@Override
	public LianmengApply findByMemberIdAndStatus(String memberId, String status) {
    ...
    @Override
    public LianmengApply findByMemberIdAndStatus(String memberId) {
        Query query = new Query();
        query.addCriteria(Criteria.where("memberId").is(memberId));
        query.with(new Sort(new Sort.Order(Sort.Direction.DESC, "createTime")));
        return mongoTemplate.findOne(query, LianmengApply.class);
    ...
    public void deleteByMemberIdAndStatus(String memberId) {
        Query query = new Query();
        query.addCriteria(Criteria.where("memberId").is(memberId));
        query.addCriteria(Criteria.where("status").is(LianmengApplyStatus.FAIL));
        query.with(new Sort(new Sort.Order(Sort.Direction.DESC, "createTime")));
        mongoTemplate.remove(query, LianmengApply.class);
    }
