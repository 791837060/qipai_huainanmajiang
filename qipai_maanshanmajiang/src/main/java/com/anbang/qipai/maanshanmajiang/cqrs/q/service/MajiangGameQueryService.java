package com.anbang.qipai.maanshanmajiang.cqrs.q.service;
...
import com.anbang.qipai.maanshanmajiang.cqrs.q.dao.*;
...
import com.anbang.qipai.maanshanmajiang.plan.dao.PlayerInfoDao;
...
public class MajiangGameQueryService {
    ...
    public void backToGame(String playerId, MajiangGameValueObject majiangGameValueObject) {
        majiangGameDboDao.updatePlayerOnlineState(majiangGameValueObject.getId(), playerId, majiangGameValueObject.findPlayerOnlineState(playerId));
    ...
    public void leaveGame(MajiangGameValueObject majiangGame) {
        ...
        if (majiangGame.getJuResult() != null) {
            MaanshanMajiangJuResult maanshanMajiangJuResult = (MaanshanMajiangJuResult) majiangGame.getJuResult();
            JuResultDbo juResultDbo = new JuResultDbo(majiangGame.getId(), null, maanshanMajiangJuResult);
    ...
    public void finishGameImmediately(MajiangGameValueObject majiangGameValueObject) {
        Map<String, PlayerInfo> playerInfoMap = new HashMap<>();
        majiangGameValueObject.allPlayerIds().forEach((playerId) -> playerInfoMap.put(playerId, playerInfoDao.findById(playerId)));
        ...
        if (majiangGameValueObject.getJuResult() != null) {
            MaanshanMajiangJuResult maanshanMajiangJuResult = (MaanshanMajiangJuResult) majiangGameValueObject.getJuResult();
            JuResultDbo juResultDbo = new JuResultDbo(majiangGameValueObject.getId(), null, maanshanMajiangJuResult);
            juResultDboDao.save(juResultDbo);
        }
    }
    ...
    public void finishGameImmediately(MajiangGameValueObject majiangGameValueObject, PanResultDbo panResultDbo) {
        ...
        if (majiangGameValueObject.getJuResult() != null) {
            MaanshanMajiangJuResult maanshanMajiangJuResult = (MaanshanMajiangJuResult) majiangGameValueObject.getJuResult();
            JuResultDbo juResultDbo = new JuResultDbo(majiangGameValueObject.getId(), panResultDbo, maanshanMajiangJuResult);
            juResultDboDao.save(juResultDbo);
        }
    }
    ...
    public boolean findByPlayerId(String gameId, String playerId) {
        return watchRecordDao.findByPlayerId(gameId, playerId, "join") != null;
    }
}
