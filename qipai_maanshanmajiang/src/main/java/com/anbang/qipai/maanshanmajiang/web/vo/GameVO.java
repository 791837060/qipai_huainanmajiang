...
public class GameVO {
    ...
    private List<String> tuoguanPlayerIds = new ArrayList<>();//监控托管玩家ID
    /**
     * 当前倒
     */
    ...
    public GameVO(MajiangGameDbo majiangGameDbo) {
        ...
        majiangGameDbo.getPlayers().forEach((dbo) -> playerList.add(new MajiangGamePlayerVO(dbo, majiangGameDbo)));
        ...
        if (sn.equals(Canceled.name)) {
            ...
            state = "waitingStart";
        } else if (sn.equals(DingqueState.name)) {
            state = "dingque";
        } else if (sn.equals(VotingWhenDingque.name)) {
            state = "dingque";
        } else if (sn.equals(VoteNotPassWhenDingque.name)) {
            state = "dingque";
        } else {
        }
        currentDao = majiangGameDbo.getCurrentDao();
    }
