...
import com.anbang.qipai.maanshanmajiang.cqrs.c.domain.dingque.VoteNotPassWhenDingque;
import com.anbang.qipai.maanshanmajiang.cqrs.c.domain.dingque.VotingWhenDingque;
...
public class MajiangPlayQueryService {
    ...
    public PanActionFrame findAndFilterCurrentPanValueObjectForPlayer(String gameId, String playerId) throws Exception {
        ...
        if (!(majiangGameDbo.getState().name().equals(Playing.name)
                ...
                || majiangGameDbo.getState().name().equals(VotingWhenDingque.name)
                || majiangGameDbo.getState().name().equals(VoteNotPassWhenDingque.name)
                || majiangGameDbo.getState().name().equals(DingqueState.name))) {
    ...
    public void readyForGame(ReadyForGameResult readyForGameResult) {
        ...
        if (majiangGame.getState().name().equals(Playing.name) || majiangGame.getState().name().equals(DingqueState.name)) {
    ...
    public void readyToNextPan(ReadyToNextPanResult readyToNextPanResult) {
        ...
        if (readyToNextPanResult.getFirstActionFrame() != null) {
            gameLatestPanActionFrameDboDao.save(majiangGame.getId(), readyToNextPanResult.getFirstActionFrame());
            // 记录一条Frame，回放的时候要做
            String gameId = majiangGame.getId();
    ...
    public void action(MajiangActionResult majiangActionResult) {
        ...
        // 盘出结果的话要记录结果
        MaanshanMajiangPanResult maanshanMajiangPanResult = majiangActionResult.getPanResult();
        if (maanshanMajiangPanResult != null) {
            PanResultDbo panResultDbo = new PanResultDbo(gameId, maanshanMajiangPanResult);
