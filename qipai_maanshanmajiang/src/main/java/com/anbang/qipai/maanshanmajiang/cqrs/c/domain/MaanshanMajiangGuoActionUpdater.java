...
public class MaanshanMajiangGuoActionUpdater implements MajiangPlayerGuoActionUpdater {
    ...
    public void updateActions(MajiangGuoAction guoAction, Ju ju) {
        ...
        if (action.getType().equals(MajiangPlayerActionType.mo)) { //过的是我摸牌之后的胡,杠
            ...
            if (currentPan.allPlayerHasNoActionCandidates() && !currentPan.anyPlayerHu()) {// 如果所有玩家啥也干不了
                MaanshanMajiangPengGangActionStatisticsListener pengGangRecordListener = ju.getActionStatisticsListenerManager().findListener(MaanshanMajiangPengGangActionStatisticsListener.class);
        ...
        } else if (action.getType().equals(MajiangPlayerActionType.dingque)) {
            if (player.getActionCandidates().isEmpty()) {
                player.generateDaQuepaiActions();
                if (player.getActionCandidates().isEmpty()){
                    player.generateDaGuipaiActions();
