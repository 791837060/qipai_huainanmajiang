...
import com.anbang.qipai.game.msg.channel.sink.TaizhouMajiangResultSink;
...
import com.anbang.qipai.game.plan.bean.historicalresult.majiang.TaizhouMajiangJuPlayerResult;
import com.anbang.qipai.game.plan.bean.historicalresult.majiang.TaizhouMajiangPanPlayerResult;
...
@EnableBinding(TaizhouMajiangResultSink.class)
public class TaizhouMajiangResultMsgReceiver {
    @Autowired
    ...
    @StreamListener(TaizhouMajiangResultSink.TAIZHOUMAJIANGRESULT)
    public void recordMajiangHistoricalResult(CommonMO mo) {
        String msg = mo.getMsg();
        String json = gson.toJson(mo.getData());
        Map map = gson.fromJson(json, Map.class);
        if ("taizhouMajiang ju result".equals(msg)) {
            ...
            if (gid != null && dyjId != null && dthId != null) {
                ...
                if (room != null) {
                    ...
                    majiangHistoricalResult.setGame(Game.taizhouMajiang);
                    ...
                    if (playerList != null) {
                        ...
                        ((List) map.get("playerResultList")).forEach((juPlayerResult) -> juPlayerResultList.add(new TaizhouMajiangJuPlayerResult((Map) juPlayerResult)));
                        ...
                        memberLatAndLonService.taizhouMajiangDels(juPlayerResultList);
                        majiangHistoricalResultService.addGameHistoricalResult(majiangHistoricalResult);
                    }
                }
            }
        }
        if ("taizhouMajiang pan result".equals(msg)) {
            Object gid = map.get("gameId");
            if (gid != null) {
                String gameId = (String) gid;
                GameRoom room = gameService.findRoomByGameAndServerGameGameId(Game.taizhouMajiang, gameId);
                if (room != null) {
                    ...
                    if (playerList != null) {
                        ...
                        ((List) playerList).forEach((panPlayerResult) -> panPlayerResultList.add(new TaizhouMajiangPanPlayerResult((Map) panPlayerResult)));
