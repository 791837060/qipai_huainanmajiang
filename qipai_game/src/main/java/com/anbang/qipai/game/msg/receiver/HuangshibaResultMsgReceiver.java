...
import com.anbang.qipai.game.msg.channel.sink.HuangshibaResultSink;
...
import com.anbang.qipai.game.plan.bean.historicalresult.zipai.HuangshibaPanPlayerResult;
...
@EnableBinding(HuangshibaResultSink.class)
public class HuangshibaResultMsgReceiver {
	@Autowired
	...
	@StreamListener(HuangshibaResultSink.HUANGSHIBARESULT)
	public void recordMajiangHistoricalResult(CommonMO mo) {
		String msg = mo.getMsg();
		String json = gson.toJson(mo.getData());
		Map map = gson.fromJson(json, Map.class);
		if ("huangshiba ju result".equals(msg)) {
			...
			if (gid != null && dyjId != null && dthId != null) {
				...
				GameRoom room = gameService.findRoomByGameAndServerGameGameId(Game.huangshiba, gameId);
				if (room != null) {
					...
					majiangHistoricalResult.setGame(Game.huangshiba);
					...
					if (playerList != null) {
						...
						((List) playerList).forEach((juPlayerResult) -> juPlayerResultList.add(new HuangshibaJuPlayerResult((Map) juPlayerResult)));
						...
						memberLatAndLonService.huangshibaDels(juPlayerResultList);
						majiangHistoricalResultService.addGameHistoricalResult(majiangHistoricalResult);
					}
				}
			}
		}
		if ("huangshiba pan result".equals(msg)) {
			Object gid = map.get("gameId");
			if (gid != null) {
				String gameId = (String) gid;
				GameRoom room = gameService.findRoomByGameAndServerGameGameId(Game.huangshiba, gameId);
				if (room != null) {
					...
					if (playerList != null) {
						...
						((List) map.get("playerResultList")).forEach((panPlayerResult) -> panPlayerResultList.add(new HuangshibaPanPlayerResult((Map) panPlayerResult)));
