...
import com.anbang.qipai.game.msg.channel.sink.MaanshanMajiangResultSink;
...
import com.anbang.qipai.game.plan.bean.historicalresult.majiang.MaanshanMajiangJuPlayerResult;
import com.anbang.qipai.game.plan.bean.historicalresult.majiang.MaanshanMajiangPanPlayerResult;
...
@EnableBinding(MaanshanMajiangResultSink.class)
public class MaanshanMajiangResultMsgReceiver {
    @Autowired
    ...
    @StreamListener(MaanshanMajiangResultSink.MAANSHANMAJIANGRESULT)
    public void recordMajiangHistoricalResult(CommonMO mo) {
        String msg = mo.getMsg();
        String json = gson.toJson(mo.getData());
        Map map = gson.fromJson(json, Map.class);
        if ("maanshanMajiang ju result".equals(msg)) {
            ...
            if (gid != null && dyjId != null && dthId != null) {
                ...
                GameRoom room = gameService.findRoomByGameAndServerGameGameId(Game.maanshanMajiang, gameId);
                if (room != null) {
                    ...
                    majiangHistoricalResult.setGame(Game.maanshanMajiang);
                    ...
                    if (playerList != null) {
                        ...
                        ((List) map.get("playerResultList")).forEach((juPlayerResult) -> juPlayerResultList.add(new MaanshanMajiangJuPlayerResult((Map) juPlayerResult)));
                        ...
                        memberLatAndLonService.maanshanMajiangDels(juPlayerResultList);
                        majiangHistoricalResultService.addGameHistoricalResult(majiangHistoricalResult);
                    }
                }
            }
        }
        if ("maanshanMajiang pan result".equals(msg)) {
            Object gid = map.get("gameId");
            if (gid != null) {
                String gameId = (String) gid;
                GameRoom room = gameService.findRoomByGameAndServerGameGameId(Game.maanshanMajiang, gameId);
                if (room != null) {
                    ...
                    if (playerList != null) {
                        ...
                        ((List) playerList).forEach((panPlayerResult) -> panPlayerResultList.add(new MaanshanMajiangPanPlayerResult((Map) panPlayerResult)));
