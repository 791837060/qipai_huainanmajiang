...
import com.anbang.qipai.game.msg.channel.sink.BohuGameSink;
...
@EnableBinding(BohuGameSink.class)
public class BohuGameMsgReceiver {
    @Autowired
    ...
    @StreamListener(BohuGameSink.BOHUGAME)
    public void receive(CommonMO mo) {
        ...
        if ("playerQuit".equals(msg)) {// 有人退出游戏
            ...
            if (room != null) {
                ...
                gameService.playerQuitGame(Game.bohu,gameId, playerId);
            }
        }
        if ("ju canceled".equals(msg)) {// 取消游戏
            Map data = (Map) mo.getData();
            String gameId = (String) data.get("gameId");
            GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.bohu, gameId);
            if (gameRoom != null) {
                ...
                gameService.gameRoomFinished(Game.bohu, gameId);
            }
        }
        ...
        if ("pan finished".equals(msg)) {// 一盘游戏结束
            ...
            gameService.panFinished(Game.bohu, gameId, no, playerIds);
        }
        if ("game delay".equals(msg)) {// 游戏延时
            ...
            // 延时11小时
            if (gameRoom != null) {
                gameService.delayGameRoom(Game.bohu, gameId, gameRoom.getDeadlineTime() + 11 * 60 * 60 * 1000);
            }
        }
    }
}
