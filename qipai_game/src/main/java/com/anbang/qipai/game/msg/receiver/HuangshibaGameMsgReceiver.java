...
import com.anbang.qipai.game.msg.channel.sink.HuangshibaGameSink;
...
@EnableBinding(HuangshibaGameSink.class)
public class HuangshibaGameMsgReceiver {
    @Autowired
    ...
    @StreamListener(HuangshibaGameSink.HUANGSHIBAGAME)
    public void receive(CommonMO mo) {
        ...
        if ("playerQuit".equals(msg)) {// 有人退出游戏
            ...
            if (room != null) {
                ...
                gameService.playerQuitGame(Game.huangshiba,gameId, playerId);
            }
        }
        if ("ju canceled".equals(msg)) {// 取消游戏
            Map data = (Map) mo.getData();
            String gameId = (String) data.get("gameId");
            GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.huangshiba, gameId);
            if (gameRoom != null) {
                ...
                gameService.gameRoomFinished(Game.huangshiba, gameId);
            }
        }
        ...
        if ("pan finished".equals(msg)) {// 一盘游戏结束
            ...
            gameService.panFinished(Game.huangshiba, gameId, no, playerIds);
        }
        if ("game delay".equals(msg)) {// 游戏延时
            ...
            // 延时11小时
            if (gameRoom != null) {
                gameService.delayGameRoom(Game.huangshiba, gameId, gameRoom.getDeadlineTime() + 11 * 60 * 60 * 1000);
            }
        }
    }
}
