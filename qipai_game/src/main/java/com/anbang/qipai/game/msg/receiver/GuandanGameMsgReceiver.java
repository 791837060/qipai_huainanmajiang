...
import com.anbang.qipai.game.msg.channel.sink.GuandanGameSink;
    ...
    public void receive(CommonMO mo) {
        ...
        if ("playerQuit".equals(msg)) {// 有人退出游戏
            ...
            if (room != null) {
                ...
                gameService.playerQuitGame(Game.guandan,gameId, playerId);
            }
        }
        if ("ju canceled".equals(msg)) {// 取消游戏
            Map data = (Map) mo.getData();
            String gameId = (String) data.get("gameId");
            GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.guandan, gameId);
            if (gameRoom != null) {
                ...
                gameService.gameRoomFinished(Game.guandan, gameId);
            }
        }
        ...
        if ("pan finished".equals(msg)) {// 一盘游戏结束
            ...
            gameService.panFinished(Game.guandan, gameId, no, playerIds);
        }
        if ("game delay".equals(msg)) {// 游戏延时
            ...
            // 延时11小时
            if (gameRoom != null) {
                gameService.delayGameRoom(Game.guandan, gameId, gameRoom.getDeadlineTime() + 11 * 60 * 60 * 1000);
            }
        }
    }
