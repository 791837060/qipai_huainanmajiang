...
import com.anbang.qipai.game.msg.channel.sink.YizhengMajiangGameSink;
...
public class YizhengMajiangGameMsgReveiver {
    ...
    public void receive(CommonMO mo) {
        ...
        if ("ju canceled".equals(msg)) {// 取消游戏
            Map data = (Map) mo.getData();
            String gameId = (String) data.get("gameId");
            GameRoom gameRoom = gameService.findRoomByGameAndServerGameGameId(Game.yizhengMajiang, gameId);
            if (gameRoom != null) {
                ...
                gameService.gameRoomFinished(Game.yizhengMajiang, gameId);
            }
        }
        ...
        if ("pan finished".equals(msg)) {// 一盘游戏结束
            ...
            gameService.panFinished(Game.yizhengMajiang, gameId, no, playerIds);
        }
        if ("game delay".equals(msg)) {// 游戏延时
            ...
            if (gameRoom != null) {
                // 延时11小时
                gameService.delayGameRoom(Game.yizhengMajiang, gameId, gameRoom.getDeadlineTime() + 11 * 60 * 60 * 1000);
            }
        }
    }
